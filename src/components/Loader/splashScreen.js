...
export default function SplashScreen() {
  const craigControls = useAnimation();
  const linkeControls = useAnimation();
  ...
  const [craigTargetX, setCraigTargetX] = useState(0);
  const [linkeTargetX, setLinkeTargetX] = useState(0);
  const [hasCalculatedTargets, setHasCalculatedTargets] = useState(false);

  const calculateTargetPositions = useCallback(() => {
    if (craigRef.current && linkeRef.current) {
      const screenWidth = window.innerWidth;

      const craigRect = craigRef.current.getBoundingClientRect();
      const linkeRect = linkeRef.current.getBoundingClientRect();

      const targetCraigLeft = EDGE_PADDING;
      const moveCraigX = targetCraigLeft - craigRect.left;
      setCraigTargetX(moveCraigX);

      const targetLinkeRight = screenWidth - 34;
      const moveLinkeX = targetLinkeRight - linkeRect.right;
      setLinkeTargetX(moveLinkeX);

      setHasCalculatedTargets(true);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('resize', calculateTargetPositions);

    return () => {
      isMounted.current = false;
      window.removeEventListener('resize', calculateTargetPositions);
    };
  }, [calculateTargetPositions]);

  useEffect(() => {
    if (!isMounted.current || !hasCalculatedTargets) return;
  ...
  }, [craigControls, linkeControls, logoControls, craigTargetX, linkeTargetX, hasCalculatedTargets]);
  return (
    <>
