...
export const PROJECTS_QUERY_PREVIEW = gql`
  query Projects {
...
export const GET_BY_SLUG_JOURNALS_PREVIEW = gql`
  query GetBySlugJournals($slug: String!) {
    journals(status: DRAFT, filters: { Slug: { eq: $slug } }) {
      Name
      Slug
      Description
      Media {
...
export const PROJECTS_QUERY_SLUG_PREVIEW = gql`
  query GetCurrentAndNextProject($slug: String!) {
    projects(status: DRAFT, filters: { Slug: { eq: $slug } }) {
      Name
      Slug
      Media {
        EnableMuxVideo
...
const ASTHETICS_BLOCK_SELECTION = `
...
export const ASTHETICS_QUERY_SLUG_PREVIEW = gql `
  query AstheticsDetails {
    astheticsDetails(status: DRAFT) {
      Name
      Slug
      Description
      DesktopMedia {
